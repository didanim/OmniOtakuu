...
document.addEventListener('click', function(e) {
  ...
  if (citeElement) {
    ...
    const commentWrapper = citeElement.closest('.comment-inner, .comment-header, .message-cell-user');
    ...
    if (profileId) {
      let views = parseInt(localStorage.getItem('view_' + profileId) || 0) + 1;
...
async function showProfileModal(name, url, img, id, views, badge) {
  ...
  bContainer.style.display = badge ? 'block' : 'none';
